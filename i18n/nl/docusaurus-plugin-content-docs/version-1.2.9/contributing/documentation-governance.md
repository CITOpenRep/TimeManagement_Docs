...
Maak een nieuw `.md` (of `.mdx`) bestand onder de juiste submap van `docs/`:
* `docs/user/`: Voor gebruikershandleidingen, installatiehandleidingen en probleemoplossing voor gebruikers.
* `docs/technical/`: Voor architectuurontwerp, organisatie van de codebase, databaseschema's en releasestappen.
* `docs/contributing/`: Voor ontwikkelaarsworkflows, richtlijnen en beleidsdocumenten.
...
### Step 2: Configureer de zijbalknavigatie
De navigatiestructuur van de zijbalk is gedefinieerd in `sidebars.js`.

1. Open `sidebars.js`.
2. Zoek de juiste categorie (bijv. `Functional`, `Technical` of `Contributor`).
3. Voeg het relatieve pad van uw bestand (exclusief het prefix `docs/` en de extensie `.md`) toe aan de `items`-array:
...
:::warning
Als u documentatie toevoegt aan een gearchiveerde releaseversie (zoals versie `1.2.9`), bewerk dan geen bestanden in `docs/` of `sidebars.js`. Zie in plaats daarvan [Deel 3: Documentversionering](#deel-3-documentversionering) hieronder.
:::
...
* **Volgende (Actieve Ontwikkeling)**: Verwijst naar bestanden die zich momenteel in `docs/` bevinden en zijn geconfigureerd in `sidebars.js`.
* **Gearchiveerde versies (bijv. 1.2.9)**: Bevroren momentopnamen opgeslagen onder `versioned_docs/version-1.2.9/` en zijbalkconfiguratie `versioned_sidebars/version-1.2.9-sidebars.json`.

### Levenscyclusdiagram

```mermaid
flowchart TD
    docs[docs/ <br>Actieve 'Volgende' Docs] -- Run docs:version command --> archive[versioned_docs/version-X.Y.Z/ <br>Bevroren momentopname]
    sidebars[sidebars.js <br>Actieve Zijbalk] -- Run docs:version command --> archive_sb[versioned_sidebars/version-X.Y.Z-sidebars.json <br>Bevroren zijbalk]
    versions[versions.json <br>Versie-index] -- Automatisch bijgewerkt --> list[Lijst versie X.Y.Z]
```

### Geversioneerde documenten bewerken (Versie 1.2.9)
Om documentatie specifiek voor versie `1.2.9` toe te voegen of te wijzigen:
1. **Bestand toevoegen/bewerken**: Maak of wijzig het bestand onder `versioned_docs/version-1.2.9/contributing/` (of andere submappen).
2. **De geversioneerde zijbalk bijwerken**: Bewerk de JSON-configuratie in `versioned_sidebars/version-1.2.9-sidebars.json` to register the file ID.
...
De website ondersteunt momenteel Engels (`en`, bron) en Nederlands (`nl`, vertaling). Alle vertaalactiva zijn opgeslagen in de map `i18n/nl/`.
...
|---|---|
| `docs/contributing/guide.md` (Volgende Docs) | `i18n/nl/docusaurus-plugin-content-docs/current/contributing/guide.md` |
| `versioned_docs/version-1.2.9/contributing/guide.md` | `i18n/nl/docusaurus-plugin-content-docs/version-1.2.9/contributing/guide.md` |
| `src/pages/index.js` (Statische React-pagina) | `i18n/nl/docusaurus-plugin-content-pages/index.js` |
...
2. **JSON-vermeldingen vertalen**: Open en update de waarde `"message"` in:
   * `i18n/nl/code.json` (themamodules, navigatie-items voor kop-/voettekst)
   * `i18n/nl/docusaurus-plugin-content-docs/current.json` (volgende zijbalkcategorieën)
   * `i18n/nl/docusaurus-plugin-content-docs/version-1.2.9.json` (versie 1.2.9 zijbalkcategorieën)
...
Voer dit uit in de hoofdmap van de repository om Docusaurus in te stellen:
```bash
npm install
```

### 2. Start de lokale ontwikkelserver
* **Preview Engels**: `npm run start` (beschikbaar op `http://localhost:3000/TimeManagement_Docs/`)
* **Preview Nederlands**: `npm run start -- --locale nl` (beschikbaar op `http://localhost:3000/TimeManagement_Docs/nl/`)
