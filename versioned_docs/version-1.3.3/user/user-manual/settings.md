...
### Floating Sync Widget & Live Progress Indicator
During any active synchronization (manual or background), a redesigned floating sync widget appears at the bottom of the screen:
* **Native Dark Styling**: Displays as a sleek, native dark floating card with rounded corners.
* **Live Percentage Progress**: Features real-time numeric progress (e.g., `30%`) so you can track exact transfer status at a glance.
