...
Create a new `.md` (or `.mdx`) file under the appropriate subdirectory of `docs/`:
* `docs/user/`: For user guides, installation manuals, and user-facing troubleshooting.
* `docs/technical/`: For architecture design, codebase organization, database schemas, and release steps.
* `docs/contributing/`: For developer workflows, guidelines, and governance documents.
...
The sidebar navigation layout is defined in `sidebars.js`.

1. Open `sidebars.js`.
2. Locate the appropriate category (e.g., `Functional`, `Technical`, or `Contributor`).
3. Add the relative path of your file (excluding the `docs/` prefix and `.md` extension) to the `items` array:
...
:::warning
If you are adding documentation to an archived release version (like version `1.2.9`), do **not** edit files in `docs/` or `sidebars.js`. Instead, see [Part 3: Document Versioning](#part-3-document-versioning) below.
:::
...
* **Next (Active Development)**: Refers to files currently inside `docs/` and configured in `sidebars.js`.
* **Archived Versions (e.g., 1.2.9)**: Frozen snapshots stored under `versioned_docs/version-1.2.9/` and sidebar config `versioned_sidebars/version-1.2.9-sidebars.json`.

### Lifecycle Diagram

```mermaid
flowchart TD
    docs[docs/ <br>Active 'Next' Docs] -- Run docs:version command --> archive[versioned_docs/version-X.Y.Z/ <br>Frozen snapshot copy]
    sidebars[sidebars.js <br>Active Sidebar] -- Run docs:version command --> archive_sb[versioned_sidebars/version-X.Y.Z-sidebars.json <br>Frozen sidebar copy]
    versions[versions.json <br>Version Index] -- Automatically updated --> list[Lists version X.Y.Z]
```

### Modifying Versioned Docs (Version 1.2.9)
To add or modify documentation specifically for version `1.2.9`:
1. **Add/Edit the File**: Create or modify the file under `versioned_docs/version-1.2.9/contributing/` (or other subdirectories).
2. **Update the Versioned Sidebar**: Edit the JSON configuration in `versioned_sidebars/version-1.2.9-sidebars.json` to register the file ID.
...
The website currently supports English (`en`, source) and Dutch (`nl`, translation). All translation assets are stored in the `i18n/nl/` folder.
...
|---|---|
| `docs/contributing/guide.md` (Next Docs) | `i18n/nl/docusaurus-plugin-content-docs/current/contributing/guide.md` |
...
| `src/pages/index.js` (Static React page) | `i18n/nl/docusaurus-plugin-content-pages/index.js` |
...
2. **Translate JSON entries**: Open and update the `"message"` value in:
   * `i18n/nl/code.json` (theme elements, header/footer navigation items)
   * `i18n/nl/docusaurus-plugin-content-docs/current.json` (Next sidebar categories)
   * `i18n/nl/docusaurus-plugin-content-docs/version-1.2.9.json` (Version 1.2.9 sidebar categories)
...
Run this in the repository root directory to set up Docusaurus:
```bash
npm install
```

### 2. Start the Local Development Server
* **Preview English**: `npm run start` (available at `http://localhost:3000/TimeManagement_Docs/`)
* **Preview Dutch**: `npm run start -- --locale nl` (available at `http://localhost:3000/TimeManagement_Docs/nl/`)
